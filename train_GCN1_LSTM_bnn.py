...
class GCNBiLSTM(nn.Module):
    def __init__(self,batch_size, feature_number, window_size, num_lstm_hidden_units, gcn_dim_h, num_classes):
        ...
        # self.conv1 = GraphConvolution(self.gnn_num_nodefeatures, gcn_dim_h)
    ...
    def forward(self, batch_x,batch_y,batch_adj,criterion):
        batch_x = batch_x.view(batch_x.size(0),-1,self.gnn_num_nodefeatures)#保持原始数据的batch_size(第一维度不变)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='GCN_SLTM_bnn motion recognization')
    ...
    if args.train_newmodel == False:
        try:
            print("load existing gcn_bilstm_bnn_model")
            model.load_state_dict(torch.load('gcn_bilstm_bnn_model_params.pth',weights_only=True))#从文件加载 cnn_lstm model
        except FileNotFoundError as e:
            print(f"未找到gcn_bilstm_bnn model文件: {e}")
    ...
    for epoch in range(num_epochs):
        ...
        for iter, (batch_x, batch_y) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            # outputs,loss = model(batch_x,batch_y,batch_adj,criterion)
        ...
        if validate_loss < best_val_loss:
            ...
            no_improve = 0
            # 保存模型 # 保存最佳模型而不是最后模型
            torch.save(model, "gcn_bilstm_bnn_model_all.pth")#保存完整模型
            torch.save(model.state_dict(), "gcn_bilstm_bnn_model_params.pth")#只保存模型参数(state_dict)
        else:
            no_improve += 1
    ...
    # 测试模型
    model_test = GCNBiLSTM(batch_size, feature_number, window_size, num_lstm_hidden_units, gnn_dim_h, num_classes).to(device)#加载最优模型
    try:
        model_test.load_state_dict(torch.load('gcn_bilstm_bnn_model_params.pth',weights_only=True))#从文件加载 cnn_lstm model
    except FileNotFoundError as e:
        print(f"未找到gcn_bilstm_bnn model文件: {e}")
    model_test.eval()
    ...
    with torch.no_grad():
        for batch_x,batch_y in test_loader:
            ...
            #     data_list.append(graph_data)
            # gcn_batch = Batch.from_data_list(data_list).to(device)
            # outputs,loss = model_test(batch_x,batch_y,batch_adj,criterion)
