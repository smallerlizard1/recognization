...
class SerialCommunication:
    def __init__(self,port_name=None,baud_rate=115200,frame_size=55,header_byte=0x55):
        self.serial_port = None
        self.is_running = False
        self.receive_thread = None
        self.buffer = bytearray()  # 初始化缓冲区
        self.newframes = []  # 最新的数据帧
        self.frame_size = frame_size  # 固定帧长度
        self.header_byte = header_byte  # 帧头标识
    ...
    def _process_frames_v2(self):
        ...
        while True:
            ...
            # 5. 提取完整帧
            if len(self.buffer) >= self.frame_size:
                self.newframes.append(self.buffer[:self.frame_size]) #对缓冲区进行切片，取出前512字节
                self.buffer = self.buffer[self.frame_size:]#移除已处理的帧数据
                print(self.newframes[-1])
...
if __name__ == "__main__":
    ...
    serial_comm = SerialCommunication(port_name='/dev/ttyUSB0', baud_rate=460800, frame_size=512)# 新建串口对象，并设置接收回调
...
# uart485_com_t.sendBuff[0] = 0x55;
# uart485_com_t.sendBuff[1] = 0x4a;
# uart485_com_t.sendBuff[2] = ADDRESS_SLAVE_485;//0x31
# uart485_com_t.sendBuff[3] = (uint8_t)((sizeof(exoskeleton_sensor_t)&0x0000ff00)>>8);
# uart485_com_t.sendBuff[4] = (uint8_t)(sizeof(exoskeleton_sensor_t)&0x000000ff);
# memcpy(uart485_com_t.sendBuff+5,(uint8_t*)&sensor_t,sizeof(exoskeleton_sensor_t));
# for(uint16_t i=0;i<(uint16_t)sizeof(exoskeleton_sensor_t)+5;i++)sum+=uart485_com_t.sendBuff[i];
# uart485_com_t.sendBuff[(uint16_t)sizeof(exoskeleton_sensor_t)+5] = sum;
# HAL_GPIO_WritePin(GPIOD, GPIO_PIN_7, GPIO_PIN_SET);//send
# //				hal_status_485 = HAL_UART_Transmit_DMA(&huart2,mysendtemp,5);//uart485_com_t.sendBuff //sizeof(exoskeleton_sensor_t)+6
# hal_status_485 = HAL_UART_Transmit_DMA(&huart2,uart485_com_t.sendBuff,sizeof(exoskeleton_sensor_t)+6);