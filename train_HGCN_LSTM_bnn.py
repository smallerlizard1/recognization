...
class HGCNBiLSTM(nn.Module):
    def __init__(self,cfg,anth,batch_size, feature_number, window_size, num_lstm_hidden_units, gcn_dim_h, num_classes):
        ...
        self.linear_p = torch.nn.Linear(self.gnn_num_nodefeatures, self.gnn_dim_h, bias = False)
        self.linear_q = torch.nn.Linear(self.gnn_num_nodefeatures, self.gnn_dim_h, bias = False)
    ...
    def forward(self, gcn_batch_x,batch_y,criterion):
        ...
        x = gcn_batch_x.x.view(self.batch_size,self.gnn_num_node,-1)
        # print(x.shape) # torch.Size([32, 100, 5])
        # print(gcn_batch_x.x.shape)
        p = self.linear_p(x)
        q = self.linear_q(x)

        for layer in self.layers[:-1]:
            la_up, la_down, actvn_up, actvn_down = layer
            p,q = la_up(p,q,gcn_batch_x.edge_index)
            p,q = la_down(p,q,gcn_batch_x.edge_index)
            p,q = actvn_up(p,q)
            p,q = actvn_down(p,q)
        ...
        h = q.view(self.batch_size,self.window_size,-1)
...
def main():
    ...
    #opt = vars(args) # Convert to dictionary # {'dataset': 'wdh.txt', 'epochs': 10, 'train_newmodel': False}
    ...
    print(edges1_index) # tensor([[0, 1, 0, 3], [1, 2, 3, 4]]) 一行展示所有源节点，一行展示所有目标节点

    # 初始化模型、损失函数和优化器
    feature_number = train_X.shape[2]
    window_size = train_X.shape[1]
    # pinoc_link5 = Pin5LinkWithIMU()
    # ctx = EnvContext()
    cfg = HFeatConfig()
    anth = HumanAnthro()
    ...
    model = HGCNBiLSTM(cfg,anth,batch_size, feature_number, window_size, num_lstm_hidden_units, gnn_dim_h, num_classes).to(device)
    ...
    writer = SummaryWriter('./logs/hgcn_bilstm1_bnn_train') # 日志保存目录
    ...
    for epoch in range(num_epochs):
        ...
        for iter, (batch_x, batch_y) in enumerate(train_loader):
            ...
            # batch_x形状: [batch_size, window_size, feature_number]32,50,25
            # batch_y形状: [batch_size]
            # gnn_imu_data = Data(batch_x,edges1_index,batch_y) #torch.Size([32, 50, 25]) batch_size, window_size, feature_number
            # 创建gcn批处理
            data_list = []
            ...
            # print(batch_x.shape) #torch.Size([32, 1, 50, 25])
            optimizer.zero_grad()
            outputs,loss = model(gcn_batch,batch_y,criterion)
    ...
    # 测试模型
    model_test = HGCNBiLSTM(cfg,anth,batch_size, feature_number, window_size, num_lstm_hidden_units, gnn_dim_h, num_classes).to(device)#加载最优模型
    try:
    ...
    with torch.no_grad():
        for batch_x,batch_y in test_loader:
            ...
            for i in range(batch_x.size(0)):
                ...
                data_list.append(graph_data)
            gcn_batch = Batch.from_data_list(data_list).to(device)
            outputs,loss = model_test(gcn_batch,batch_y,criterion)
            _, predicted = torch.max(outputs.data, 1)
...
# 模型参数量: 229560
# Epoch 0: Train Loss 1.5840, Validate Loss 0.2006, Time_perbatch 31.35ms, Train iter 4068
# Epoch 1: Train Loss 0.8376, Validate Loss 0.1882, Time_perbatch 31.80ms, Train iter 4068
# Epoch 2: Train Loss 0.7995, Validate Loss 0.0725, Time_perbatch 31.40ms, Train iter 4068
# Epoch 3: Train Loss 0.7861, Validate Loss 0.1513, Time_perbatch 31.36ms, Train iter 4068
# Epoch 4: Train Loss 0.7736, Validate Loss 0.1080, Time_perbatch 31.37ms, Train iter 4068
# Epoch 5: Train Loss 0.7654, Validate Loss 0.1407, Time_perbatch 31.41ms, Train iter 4068
# Epoch 6: Train Loss 0.7601, Validate Loss 0.1238, Time_perbatch 31.47ms, Train iter 4068
# Epoch 7: Train Loss 0.7499, Validate Loss 0.0909, Time_perbatch 31.43ms, Train iter 4068
# Epoch 8: Train Loss 0.7545, Validate Loss 0.1495, Time_perbatch 31.44ms, Train iter 4068
# Epoch 9: Train Loss 0.7356, Validate Loss 0.0755, Time_perbatch 31.43ms, Train iter 4068
# Epoch 10: Train Loss 0.7333, Validate Loss 0.0804, Time_perbatch 31.39ms, Train iter 4068
# Epoch 11: Train Loss 0.7318, Validate Loss 0.0864, Time_perbatch 31.43ms, Train iter 4068
...
# load existing gcn_bilstm_bnn_model
# 模型参数量: 229560
# Epoch 0: Train Loss 0.7823, Validate Loss 0.1923, Time_perbatch 32.36ms, Train iter 4068
# Epoch 1: Train Loss 0.7726, Validate Loss 0.1051, Time_perbatch 31.90ms, Train iter 4068
# Epoch 2: Train Loss 0.7691, Validate Loss 0.0975, Time_perbatch 32.13ms, Train iter 4068
# Epoch 3: Train Loss 0.7629, Validate Loss 0.0973, Time_perbatch 32.20ms, Train iter 4068
# Epoch 4: Train Loss 0.7564, Validate Loss 0.0896, Time_perbatch 32.12ms, Train iter 4068
# Epoch 5: Train Loss 0.7504, Validate Loss 0.1030, Time_perbatch 32.08ms, Train iter 4068
# Epoch 6: Train Loss 0.7470, Validate Loss 0.0866, Time_perbatch 32.01ms, Train iter 4068
# Epoch 7: Train Loss 0.7393, Validate Loss 0.0926, Time_perbatch 32.26ms, Train iter 4068
# Epoch 8: Train Loss 0.7389, Validate Loss 0.1087, Time_perbatch 32.01ms, Train iter 4068
# Epoch 9: Train Loss 0.7234, Validate Loss 0.1341, Time_perbatch 32.19ms, Train iter 4068
# Epoch 10: Train Loss 0.7225, Validate Loss 0.1188, Time_perbatch 32.94ms, Train iter 4068
# Epoch 11: Train Loss 0.7213, Validate Loss 0.0855, Time_perbatch 33.07ms, Train iter 4068
# Epoch 12: Train Loss 0.7165, Validate Loss 0.0995, Time_perbatch 32.58ms, Train iter 4068
# Epoch 13: Train Loss 0.7135, Validate Loss 0.1058, Time_perbatch 32.35ms, Train iter 4068
# Epoch 14: Train Loss 0.7101, Validate Loss 0.1232, Time_perbatch 32.51ms, Train iter 4068
# Epoch 15: Train Loss 0.7100, Validate Loss 0.0868, Time_perbatch 32.46ms, Train iter 4068
# Epoch 16: Train Loss 0.7054, Validate Loss 0.1419, Time_perbatch 32.40ms, Train iter 4068
# Epoch 17: Train Loss 0.7029, Validate Loss 0.1237, Time_perbatch 32.35ms, Train iter 4068
# Epoch 18: Train Loss 0.6971, Validate Loss 0.0839, Time_perbatch 32.28ms, Train iter 4068
# Epoch 19: Train Loss 0.6935, Validate Loss 0.0922, Time_perbatch 32.11ms, Train iter 4068
# Epoch 20: Train Loss 0.6950, Validate Loss 0.0946, Time_perbatch 32.99ms, Train iter 4068
# Epoch 21: Train Loss 0.6950, Validate Loss 0.0568, Time_perbatch 32.19ms, Train iter 4068
# Epoch 22: Train Loss 0.6966, Validate Loss 0.0829, Time_perbatch 32.36ms, Train iter 4068
# Epoch 23: Train Loss 0.6960, Validate Loss 0.0656, Time_perbatch 32.19ms, Train iter 4068
# Epoch 24: Train Loss 0.6948, Validate Loss 0.1113, Time_perbatch 32.71ms, Train iter 4068
# Epoch 25: Train Loss 0.6947, Validate Loss 0.1169, Time_perbatch 32.35ms, Train iter 4068
# Epoch 26: Train Loss 0.6962, Validate Loss 0.0914, Time_perbatch 32.38ms, Train iter 4068
# Epoch 27: Train Loss 0.6927, Validate Loss 0.1035, Time_perbatch 32.25ms, Train iter 4068
# Epoch 28: Train Loss 0.6880, Validate Loss 0.1028, Time_perbatch 32.42ms, Train iter 4068
# Epoch 29: Train Loss 0.6888, Validate Loss 0.0661, Time_perbatch 32.18ms, Train iter 4068
# Epoch 30: Train Loss 0.6892, Validate Loss 0.0975, Time_perbatch 32.24ms, Train iter 4068
